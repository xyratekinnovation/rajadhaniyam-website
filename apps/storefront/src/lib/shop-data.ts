...
import { formatInr, type Category, type Product } from "@rajadhaniyam/shared";

// Re-exported so existing imports (`import { Product } from "@/lib/shop-data"`)
// keep working. The canonical type definitions now live in @rajadhaniyam/shared
// so storefront, admin and API agree on one shape.
export type { Category, Product };

/**
 * This module is the MOCK data source behind productService/categoryService
 * (see src/services). It stays in place so the storefront keeps working
 * without a backend; once apps/api exposes real endpoints, only the
 * repository implementation inside services/ needs to change.
 */
