            ...
            <ul className="mt-4 space-y-2 text-sm">
              <li>
                <Link
                  to="/shop"
                  search={{ sort }}
                  className={!category ? "text-olive underline" : "text-charcoal/75 hover:text-olive"}
                >
                  All Products
                </Link>
              </li>
              {categories.map((c) => (
                <li key={c.slug}>
                  <Link
                    to="/shop"
                    search={{ category: c.slug, sort }}
                    className={
                      category === c.slug ? "text-olive underline" : "text-charcoal/75 hover:text-olive"
                    }
                  >
                    {c.name}
                  </Link>
          ...
          <div className="mb-6 flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
            ...
            <select
              ...
              onChange={(e) =>
                navigate({
                  to: ".",
                  search: (prev) => ({ ...prev, sort: e.target.value || undefined }),
                })
              }
